"""
=====================================
THE BoardGui CLASS IS A TKINTER GUI 
DISPLAY FOR THE DamaBoard CLASS
=====================================
"""
...
class DamaGui:
    ...
    def __init__(self, dama_board, game_operator = None):
        """
        creates a dama graphical interface 
        based on the given dama board object
        """
        ...
        self.cells = [
            [None for row in range(self.board.NUM_ROWS)] \
        ...
        # contains the list of enabled cells tag bind ids
        self.enableds = [] 
        self.tempEnableds = []
    ...
    def paintBoard(self):
        ...
        for row in range(self.board.NUM_ROWS):
            for col in range(self.board.NUM_ROWS):
                ...
                # draw a cell
                cellcolor = 'brown' if self.board.isIllegalCell(row, col) \
                 else 'black'
    ...
    def disableAllTemporaries(self):
        ...
        # bind_id is the id returned earlier by tag_bind
        for cell_bindId in self.tempEnableds: 
            row, col, bind_id = \
            cell_bindId[0][0], cell_bindId[0][1], cell_bindId[1]
            self.canvas.tag_unbind(
                "cell" + str(row) + str(col), 
                "<ButtonPress-1>", str(bind_id)
            )
    ...
    def cellClicked(self, event):
        ...
        clicked_cell = event.y // CELL_WIDTH, \
        event.x // CELL_WIDTH # row, col of the clicked cell
    ...
    def disableAll(self):
        ...
        # bind_id is the id returned earlier by tag_bind
        for cell_bindId in self.enableds: 
            row, col, bind_id = cell_bindId[0][0], \
            cell_bindId[0][1], cell_bindId[1]
            self.canvas.tag_unbind(
                "cell" + str(row) + str(col), 
                "<ButtonPress-1>", str(bind_id)
            )
    ...
    def drawCorki(self, row, col):
        ...
        corki = self.canvas.create_image(
            xcoor, ycoor, image = photo, 
            tag = "cell" + str(row) + str(col)
        )
        return corki

    # performs a slow gui move of a corki from 
    # start cell to its landing cell
    def moveCorki(self, start_cell, landing_cell):

        row1, col1, row2, col2 = start_cell[0], start_cell[1], \
        ...
        # so it can be seen moving on top of the canvas, 
        # and not under some rectangle
        self.canvas.lift(moving_corki) 
        ...
        # update the corki tag, now that 
        # it has moved to a different cell
        self.canvas.itemconfig(
            moving_corki, 
            tag = "cell" + str(row2) + str(col2)
        )
        ...
        if row2 == 0 and self.board.isKingBottom(row2, col2):
            self.canvas.itemconfig(
                moving_corki, 
                image = self.king_bottom_photo
            )

        if row2 == 7 and self.board.isKingTop(row2, col2):
            self.canvas.itemconfig(
                moving_corki, 
                image = self.king_top_photo
            )

        self.canvas.update()
        # wait half a second for the move to 
        # be taken in / to be noticed
        sleep(0.5)
    ...
    def animateMove(self, start_cell, landing_cell, did_eat):
        ...
        if did_eat:
            eaten_row, eaten_col = self.board.getMiddleCell(
                start_cell, landing_cell
            )
    ...
    # toggles debug mode, and enables all legal cells 
    ...
    def highlightNextMove(self, move_type):
        ...
        if self.current_debug_cell is not None:
            ...
            if self.board.isTop(cur_cell):
                moves_and_eats = self.operator.\
                comp_player.movesAndEats(cur_cell)
            else:
                moves_and_eats = self.operator.\
                human_player.movesAndEats(cur_cell)
    ...
    # shows the multiple simultaneous eats that a 
    # corki at the current cell can make.
    def showEatingSpree(self):

        paths_and_scores = self.operator.comp_player.\
        spreePathsAndScores(self.current_debug_cell)
    ...
    # pickles/serializes the current board object 
    # for after restoration
    def saveBoard(self):

        with open('board.pickle', 'wb') as f:
            # Pickle the 'data' dictionary using the 
            # highest protocol available.
            pickle.dump(self.board, f, pickle.HIGHEST_PROTOCOL)

    # loads/deserializes a saved board object and 
    # sets it as the current board object
    def restoreBoard(self):

        with open('board.pickle', 'rb') as f:

            # The protocol version used is detected 
            # automatically, so we do not have to specify it.
    ...
    # displays the (row, col) coordinates using 
    # a simple dialog window
    def displayCellCoordinates(self):
        messagebox.showinfo(
            "Cell Coordinates", 
            str(self.current_debug_cell)
        )
    ...
    def addNodeToQueue(self):
        ...
        spree_paths_and_scores = self.operator.comp_player.\
        spreePathsAndScores(self.current_debug_cell)
        ...
        self.test_cur_node = self.operator.\
        comp_player.addNodeToQueue(
            queue=self.test_bfs_queue, parent=self.test_cur_node, 
            start_cell=self.current_debug_cell,
            destn_cell=self.test_destn_cell, score=score, 
            is_eat=is_eat,spree_path=spree_path,
            played_by_comp=played_by_comp
        )
    ...
    # pops up the right sets of context menu items 
    # when the user right clicks on the board
    def popContextMenu(self, event):
        ...
        # assuming the commands are less than 10 in number
        self.context_menu.delete(0, 10) 
        # add the appropriate commands to the menu first
        self.context_menu.add_command(
            label='Toggle DEBUG', 
            command=self.toggleDebugMode
        )

        if self.debug:

            self.context_menu.add_command(
                label='Show Cell Coordinates', 
                command=self.displayCellCoordinates
            )

            self.context_menu.add_command(
                label='Build Move Tree', command=self.displayMoveTree
            ...
            if self.board.isEmptyLegalCell(clicked_cell): 
                ...
                self.context_menu.add_command(
                    label='Add Teter Bottom',
                    command=lambda corkitype='teter_bottom': \
                    self.addCorki(corkitype)
                )
                
                self.context_menu.add_command(
                    label='Add Teter Top',
                    command=lambda corkitype='teter_top': \
                    self.addCorki(corkitype)
                )

                self.context_menu.add_command(
                    label='Add King Bottom',
                    command=lambda corkitype='king_bottom': \
                    self.addCorki(corkitype)
                )

                self.context_menu.add_command(
                    label='Add King Top',
                    command=lambda corkitype='king_top': 
                    self.addCorki(corkitype)
                )

                self.context_menu.add_command(
                    label='Save Board', command=self.saveBoard
                )
                self.context_menu.add_command(
                    label='Restore Board', command=self.restoreBoard
                )
                self.context_menu.add_command(
                    label='Set As Destn Cell', command=self.setTestDestnCell
                )
                self.context_menu.add_command(
                    label='Play Final Path', command=self.playFinalPath
                )
                ...
                self.context_menu.add_command(
                    ...
                    command=lambda movetype='move': \
                    self.highlightNextMove(movetype)
                )

                self.context_menu.add_command(
                    label='Show Next Eats',
                    command=lambda movetype='eat': \
                    self.highlightNextMove(movetype)
                )

                self.context_menu.add_command(
                    label='Remove Corki', command=self.removeCorki
                )
                self.context_menu.add_command(
                    label='Show Eating Spree', command=self.showEatingSpree
                )
                self.context_menu.add_command(
                    label='Add Node To Queue', command=self.addNodeToQueue
                )
