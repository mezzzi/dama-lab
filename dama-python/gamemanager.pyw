...
class GameManager:
    ...
    def handleLandingCellClick(self, landing_cell):
        ...
        is_kingified, did_eat = self.human_player.playOneStep(self.start_cell, landing_cell)
        ...
        for i in range(len(comp_advances)):
            ...
            is_kingified = self.comp_player.playOneStep(start_cell, landing_cell, is_eat)
            # tell gui to animate the human move for the user
            self.board_gui.animateMove(start_cell, landing_cell, is_eat)
            if is_kingified: break # stop continuing spree if kingified [needs RETHINKING]
        # check if game is over
