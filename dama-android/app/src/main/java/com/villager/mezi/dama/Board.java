...
class Board {
    ...
    // which is also equal with number of columns
    ...
    private int[][] board = new int[NUM_ROWS][NUM_ROWS];
    ...
    final public void populateBoard(){
        ...
        /*
        lay out the start corkis on the board; 24 in total, 12 for top
    ...
    public void saveBoard(){
        try {
            ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream("board.dat"));
            out.writeObject(board);
        } catch (IOException ex) {
            System.err.println("IOException inside saveBoard");
        }
    }

    public void restoreBoard(){
        try {
            ObjectInputStream in = new ObjectInputStream(new FileInputStream("board.dat"));
            try {
                board = (int[][])in.readObject();
                in.close();
            } catch (ClassNotFoundException ex) {
                System.err.println("Class Not Found Exception inside restoreBoard");
            }
